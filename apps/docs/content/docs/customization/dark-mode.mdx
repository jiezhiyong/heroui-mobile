---
title: Dark mode
description: HeroUiMobile comes with two palette modes light (the default) and dark.
---

# Dark mode

HeroUiMobile supports both `light` and `dark` themes. To enable dark mode, simply add the `dark` class to your root element (`html`, `body`, or `main`).
...
> Note: We're using the `class` attribute to switch between themes, this is because HeroUiMobile uses the `className` attribute.
