---
title: Routing
description: Learn how to integrate client side routing into your HeroUiMobile application.
---

# Routing

HeroUiMobile Components such as [Tabs](/docs/components/tabs), [Listbox](/docs/components/listbox), [Dropdown](/docs/components/dropdown) and many others offer
...
The `HeroUIProvider` component configures all HeroUiMobile components within it to navigate using the client side
router you provide.

Set this up once in the `root` of your app, and any HeroUiMobile component with the `href` prop will automatically navigate
using your router.
...
All `HeroUiMobile` link components accept a `routerOptions` prop that passes options to the router's navigate function for
...
apps. `HeroUIProvider` should be rendered at the `root` of each page that includes HeroUiMobile components, or in
...
To use [TanStack Router](https://tanstack.com/router/latest) with HeroUiMobile, render HeroUiMobile's RouterProvider inside your root route.
