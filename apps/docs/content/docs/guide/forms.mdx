---
title: Forms
description: Learn how to handle forms in HeroUiMobile.
---
...
HeroUiMobile provides form components with built-in validation and styling to help users input and submit data effectively.
...
Accessible forms start with clear, descriptive labels for each field. All HeroUiMobile form components support labeling using
the Label component, which is automatically associated with the field via the id and for attributes on your behalf.

In addition, HeroUiMobile components support help text, which associates additional context with a field such as a **description**
...
HeroUiMobile form components are uncontrolled by default, but if you need to manage state in real-time, you can use the `useState` hook to make the component controlled.
...
> **Note**: The default error messages are localized by the browser based on the browser/operating system language settings. The [locale setting in HeroUiMobile Provider](/docs/api-references/heroui-provider#props) does not affect validation errors.


## Validation

Form validation is crucial for ensuring that users enter the correct data.
HeroUiMobile supports native HTML constraint validation and allows for custom validation and real-time validation.

#### Built-in validation

HeroUiMobile form components support [native HTML validation](https://developer.mozilla.org/docs/Web/HTML/Constraint_validation) attributes like `isRequired` and `minLength`.
...
You can change the form defaults for your entire app using [HeroUiMobile Provider](/docs/api-references/heroui-provider).
...
HeroUiMobile allows you to display server-side validation errors by using the `validationErrors` prop in the `Form` component.
...
HeroUiMobile supports errors from schema validation libraries like [Zod](https://zod.dev/).
...
In most cases, the built-in validation features of HeroUiMobile are sufficient. However, if you're building more complex forms or integrating HeroUiMobile components into an existing form, you can use a form library like [React Hook Form](https://react-hook-form.com/) or [Formik](https://formik.org/).

#### React Hook Form

You can integrate HeroUiMobile components using [Controller](https://react-hook-form.com/docs/usecontroller/controller).
Controller allows you to manage field values and validation errors, and reflect the validation result in HeroUiMobile components.
...
> For more information about forms in HeroUiMobile, visit the [React Aria Forms Guide](https://react-spectrum.adobe.com/react-aria/forms.html).
